# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# Canal (idocumentales) por Hernan_Ar_c
# ------------------------------------------------------------
...
def busqueda(item):
    ...
    for scrapedthumbnail, scrapedtitle, scrapedurl, scrapedyear, scrapedplot in matches:
        ...
        plot = scrapedplot
        year = scrapedyear
        itemlist.append( Item(channel=item.channel, action="findvideos" , title=title , fulltitle=title, url=url, thumbnail=thumbnail, plot=plot, contentSerieName=title, infoLabels={'year':year}))
    tmdb.set_infoLabels_itemlist(itemlist, seekTmdb = True)
