...
try:
    ...
    try:
        ...
        print('Error initialising global menu : %s' % str(e))
    ADDON = None
...
def doStandard(useScript=True):
    ...
    if useScript:
        # open menu via script to prevent animation locking up (due to bug in
        # XBMC)
        path = utils.HOME
...
def whitelisted():
    ...
    try:
        ...
        if utils.addon_enabled(addon):
            addon = utils.xbmcaddon.Addon(addon).getAddonInfo('path')
...
def doMenu(mode):
    ...
    if len(path) > 0:
        ...
        if MENU_ISEARCH:
            menu.append((GETTEXT(30054), _SEARCH))

        if MENU_IRECOMMEND:
            menu.append((GETTEXT(30088), _RECOMMEND))

        if MENU_COPY_PROPS:
            menu.append((GETTEXT(30209), _COPYITEM))
    ...
    if dialog:
        ...
        # False to allow right-click to std context menu
    ...
    if choice in [_SEARCH, _SEARCHDEF, _RECOMMEND]:
        ...
        cmd = 'ActivateWindow(%d,"plugin://%s/?mode=%d&keyword=%s&image=%s&fanart=%s&meta=%s")' % (
            window, ADDONID, mode, urllib.quote_plus(label), urllib.quote_plus(thumb), urllib.quote_plus(fanart), meta)
    ...
    if choice == _COPYITEM:
        ...
        cmd = menuUtils.getCmd(path, fanart, desc, window,
                               filename, isFolder, meta)
