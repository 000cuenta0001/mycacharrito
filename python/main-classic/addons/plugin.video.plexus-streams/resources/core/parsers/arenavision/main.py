...
def arenavision_schedule(url):
	...
	if source:
		...
		for day, month, year, hour, minute, sport, competition, event, channelsStr in events:
			...
			addDir('[B][COLOR red]{0}[/COLOR] [COLOR white]{1}[/COLOR][/B] {3} [COLOR yellow][{2}][/COLOR]'.format(timeStr, removeNonAscii(sport), removeNonAscii(competition), removeNonAscii(event)), str(streams),401,os.path.join(current_dir,"icon.png"),1,False,parser="arenavision",parserfunction="arenavision_chooser")
		return None
...
def arenavision_chooser(url):
	...
	if index > -1:
		arenavision_streams("ArenaVision {0}".format(channelList[index]), base_url + "/av" + channelList[index])
